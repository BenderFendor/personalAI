...
class ChatBot:
    ...
    def __init__(self, config_path: str = "config.json", logs_dir: str = "chat_logs"):
        ...
        self.context_calc = ContextCalculator(
            default_size=int(self.config.get("context_window_size", 8192)),
        ...
        if not existing_sessions:
            ...
            if count > 0:
                self.console.print(
                    f"[green]Indexed {count} chat sessions from {logs_dir}/[/green]"
                )
        ...
        # Lazy init provider for llama.cpp when selected
        self._llama_cpp_provider = None  # type: ignore
    ...
    def _warn_common_misconfigurations(self) -> None:
        ...
        try:
            ...
            if provider in ("ollama", "llama_cpp") and model.lower().startswith(
                "gemini"
            ):
                self.console.print(
                    "[yellow]\nWarning: Your provider is set to '"
                    + provider
                    + "' but the model is '"
                    + escape(model)
                    ...
                    "- Or keep provider '"
                    + provider
                    + "' and change 'model' to a local model (e.g., 'qwen3', 'llama3.1').\n"
                    "Edit config.json accordingly.\n[/yellow]"
                )

            if provider == "llama_cpp":
                self.console.print(
                    "[dim]Using llama.cpp server at: "
                    + escape(self.config.llama_cpp_base_url)
                    + "[/dim]"
                )
            ...
            if provider == "gemini":
                ...
                if gemini_model and gemini_model.lower() in {
                    "gemini-flash",
                    "gemini-flash-latest",
                    "gemini",
                }:
    ...
    def _get_system_prompt(self) -> str:
        ...
        return (
            ...
            "2. Use web search to get current information from the internet\n"
            "3. Use inline URL citations as provided by the tools\n"
            "4. Be critical and verify information from multiple sources and break your thought process and assumptions\n"
            "5. Give concise, accurate, and well-sourced answers based on tool results using snippets, quotes, and excerpts when needed from the sources\n"
            f"{fetch_info}"
    ...
    def save_chat_log(self) -> bool:
        ...
        if not self._dirty:
            self.console.print(
                "[dim]Nothing to save (no messages sent). Skipping log write.[/dim]"
            )
            return False

        log_path = self.logger.save_session(
            self.current_session, self.messages, self.config.get_all()
        )
    ...
    def get_context_usage(self) -> ContextUsage:
        ...
        return self.context_calc.calculate_usage(
            ollama_messages, self._get_system_prompt()
        )
    ...
    def chat(self, user_input: str) -> str:
        ...
        # Show context usage
        context_usage = self.context_calc.calculate_usage(
            ollama_messages, self._get_system_prompt()
        )
        ...
        if context_usage.percentage >= self._warn_threshold * 100:
            self.console.print(
                "[yellow]Approaching context limit. Older turns may be summarized if needed.[/yellow]"
            )
        ...
        try:
            ...
            while tool_calls and iteration < max_iterations:
                ...
                for tool_call in tool_calls:
                    ...
                    try:
                        if isinstance(tool_call, dict):
                            ...
                            if "function" in tool_call:
                                ...
                                # Some models may send partial tool calls - check for name
                                function_name = func.get("name", "")
                                arguments = func.get("arguments", "{}")
                                # If name is empty, skip this incomplete tool call
                        ...
                        self.console.print(
                            f"[red]Error parsing tool call: {e}, tool_call: {tool_call}[/red]"
                        )
                        continue
                    ...
                    tool_result = self._auto_fetch_urls(
                        function_name, tool_result, user_input
                    )

                    # Add result to messages - always include tool_call_id for proper linkage
                    if tool_call_id:
                        ...
                        ollama_messages.append({"role": "tool", "content": tool_result})
                ...
                self.console.print(
                    f"[dim]   {self.display.display_context_bar(context_usage)}[/dim]"
                )
                ...
                try:
                    if isinstance(self._last_prompt_eval_count, int):
                        ...
                        follow_usage = self.context_calc.calculate_usage(
                            ollama_messages, self._get_system_prompt()
                        )
                        self.context_calc.register_actual(
                            self._last_prompt_eval_count, follow_usage.current_tokens
                        )
            ...
            if (full_response and full_response.strip()) or (
                full_thinking and full_thinking.strip()
            ):
                self._dirty = True
            ...
            try:
                if isinstance(self._last_prompt_eval_count, int):
                    self.context_calc.register_actual(
                        self._last_prompt_eval_count, estimated_before
                    )
            finally:
                self._last_prompt_eval_count = None
    ...
    def _stream_response(self, messages: List[Dict[str, Any]]) -> tuple[str, str, list]:
        ...
        """
        # Provider switch: Gemini, llama_cpp, or legacy ollama
        if self.config.llm_provider == "gemini":
            if GeminiProvider is None:
                raise RuntimeError(
                    "Gemini provider unavailable; install langchain-google-genai."
                )
            ...
            for chunk in self._gemini_provider.stream(messages):  # type: ignore[attr-defined]
                ...
                if chunk.get("tool_calls"):
                    tool_calls = chunk[
                        "tool_calls"
                    ]  # overwrite with latest collected calls
        ...
        # llama.cpp provider
        if self.config.llm_provider == "llama_cpp":
            if LlamaCppProvider is None:
                raise RuntimeError("llama.cpp provider unavailable.")
            ...
            for chunk in self._llama_cpp_provider.chat(
                messages=messages,
                tools=self.tools if self.config.tools_enabled else None,
                stream=True,
            ):
                # llama.cpp OpenAI-compatible format: chunk["choices"][0]["delta"]
                ...
                # Handle thinking (some models like Qwen use reasoning_content)
                if isinstance(delta, dict):
                    # Qwen models use reasoning_content, others may use thinking
                    thinking = delta.get("thinking") or delta.get("reasoning_content")
                else:
                    thinking = None

                # Only show thinking header once, and buffer the thinking content
                if thinking and thinking.strip():
                    if not full_thinking and self.config.show_thinking:
                        self.console.print("\n[bold magenta]Thinking:[/bold magenta]")
                        self.console.print("[dim]" + "=" * 60 + "[/dim]")
                    full_thinking += thinking
                    if self.config.show_thinking:
                        # Buffer and print in chunks to avoid ANSI clutter
                        self.console.print(f"[dim]{thinking}[/dim]", end="")

                # Handle content - buffer and print
                content = delta.get("content")
                if content and content.strip():
                    if full_thinking and self.config.show_thinking:
                        self.console.print("\n[dim]" + "=" * 60 + "[/dim]")
                    if not full_response:
                        ...
                        if isinstance(tc, dict):
                            ...
                            if "function" in tc:
                                ...
                                if "name" in func and func["name"]:
                                    partial_tool_calls[idx]["function"]["name"] = func[
                                        "name"
                                    ]
                                if "arguments" in func and func["arguments"]:
                                    partial_tool_calls[idx]["function"][
                                        "arguments"
                                    ] += func["arguments"]
                ...
                if finish_reason == "tool_calls":
                    tool_calls = list(partial_tool_calls.values())

                # Token usage
                usage = chunk.get("usage")
                if usage:
                    try:
                        pe = int(usage.get("prompt_tokens", 0))
                        ec = int(usage.get("completion_tokens", 0))
            ...
            # Convert any accumulated partial tool calls
            if partial_tool_calls and not tool_calls:
                tool_calls = list(partial_tool_calls.values())

            # Fallback: if no structured tool_calls emitted, try to parse from text
            if not tool_calls and "```tool_calls" in full_response:
                cleaned, parsed_calls = self._parse_tool_calls_from_text(full_response)
                if parsed_calls:
                    full_response = cleaned
        ...
        # Legacy ollama provider - use llama_cpp provider (compatible)
            ...
            # Stream from llama.cpp (compatible with ollama API)
            full_thinking = ""
            full_response = ""
            tool_calls: List[Dict[str, Any]] = []
        ...
        for chunk in response:
            ...
            if "content" in message and message["content"]:
                if (
                    started_thinking
                    and not finished_thinking
                    and self.config.show_thinking
                ):
                    self.console.print("\n[dim]" + "=" * 60 + "[/dim]")
    ...
    def _load_markdown_session(self, file_path: str, session_id: str) -> bool:
        ...
        try:
            ...
            for i in range(1, len(parts), 2):
                ...
                self.messages.append(
                    Message(role=role, content=text, thinking=thinking)
                )
    ...
    def _auto_fetch_urls(
        self, tool_name: str, tool_result: str, user_query: str
    ) -> str:
        ...
        try:
            urls = self.display.extract_and_rank_urls(
                tool_result, user_query, threshold=0.6
            )
            if not urls:
                return tool_result
    ...
    def _parse_tool_calls_from_text(self, text: str) -> tuple[str, list]:
        ...
        Supports formats:
        - ```tool_calls [...]``` (custom format)
        - <tool_call>{"name": ..., "arguments": ...}</tool_call> (Qwen format)
        - <tool_call>\n{"name": ...\n"arguments": ...}\n</tool_call> (Qwen multiline)
        """
        import re
        import json

        # Try standard tool_calls block first
        ...
        # Try Qwen XML format: <tool_call>{"name": ..., "arguments": ...}</tool_call>
        qwen_pattern = r"<tool_call>\s*(\{.*?\})\s*</tool_call>"
        match = re.search(qwen_pattern, text, re.DOTALL)
        if match:
            try:
                tool_obj = json.loads(match.group(1))
                # Convert to OpenAI tool_calls format
                tool_calls = [
                    {
                        "id": f"call_{hash(tool_obj.get('name', ''))}",
                        "type": "function",
                        "function": {
                            "name": tool_obj.get("name", ""),
                            "arguments": json.dumps(tool_obj.get("arguments", {})),
                        },
                    }
                ]
                clean_text = text.replace(match.group(0), "").strip()
        ...
        # Try multiline Qwen format with newlines
        qwen_pattern2 = r"<tool_call>\s*(\{[\s\S]*?\})\s*</tool_call>"
        match = re.search(qwen_pattern2, text)
        if match:
            try:
                # Try to extract name and arguments from the JSON
                tool_obj_str = match.group(1)
                # Clean up the JSON - remove newlines in string values
                tool_obj_str = re.sub(r"(\w+):", r'"\1":', tool_obj_str)
                tool_obj = json.loads(tool_obj_str)
                tool_calls = [
                    {
                        "id": f"call_{hash(tool_obj.get('name', ''))}",
                        "type": "function",
    ...
    def _enrich_error_message(self, error: Exception) -> str | None:
        ...
        if "connection refused" in msg:
            if provider == "llama_cpp":
                return "Is the llama.cpp server running? Try 'llama-server' or check if the service is active."
            elif provider == "ollama":
                return "Is Ollama running? Try 'ollama serve' or check if the service is active."
            return "Check that the LLM server is running."
        return None
    ...
    def chat_stream(self, user_input: str) -> Generator[Dict[str, Any], None, None]:
        ...
        yield {
            ...
            "data": {
                ...
                if context_usage.percentage >= 0.7 * 100
                else "green",
            },
        }
    ...
    def _stream_generator(
        ...
        if self.config.llm_provider == "gemini":
        ...
        elif self.config.llm_provider in ("llama_cpp", "ollama"):
            # Use llama.cpp provider for both llama_cpp and legacy ollama
            if LlamaCppProvider is None:
                yield {"type": "error", "content": "llama.cpp provider unavailable"}
                return
            if self._llama_cpp_provider is None:
                base_url = (
                    "http://localhost:11434"
                    if self.config.llm_provider == "ollama"
                    else self.config.llama_cpp_base_url
                )
                self._llama_cpp_provider = LlamaCppProvider(
                    model=self.config.model,
                    base_url=base_url,
            ...
            for chunk in self._llama_cpp_provider.chat(
                ...
                # Handle thinking - Qwen uses reasoning_content
                thinking = delta.get("thinking") or delta.get("reasoning_content")
                if thinking:
            ...
            yield {
                "type": "error",
                "content": f"Unknown provider: {self.config.llm_provider}",
            }
            return
